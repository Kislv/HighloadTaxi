...
Бинарные данные: 5 мб * 100 000 000 = 100 000 Тб

У каждого пользователя примерно 2 избранных места, тогда на координаты этих мест на 1 пользователя требуется 16 байт
...
Данные с плавающей точкой: 16 байт * 100 000 000 + 2 416 байт * 500 000 000 = 1.209 Тб
...
на сервер:
- id клиента (4 байта);
- точки начала и конца маршрута (8 байт);
- цена (4 байта);
от сервера:
...
на сервер:
- центр области карты, куда смотрит клиент - 4 байта;
- масштаб карты - 4 байтаю
от сервера:
- местоположение ~4 автомобилей и их направление - 20 байт;
...
<!-- (2.16 млн * 16 байт + 11.04 млн * 8 байт + 4.56 млн * 12 байт) / 24 / 60 / 60 с * 8 * 2-->
<!-- (2.16 млн * 1 байт + 11.04 млн * 20 байт + 4.56 млн * 12 байт) / 24 / 60 / 60 с * 8 * 2 -->
Пиковое потребление в течении суток (Гбит/c):
|на сервер|от сервера|
|--|--|
|0.03|0.05|

<!-- (2.16 млн * 16 байт + 11.04 млн * 8 байт + 4.56 млн * 12 байт)  -->
<!-- (2.16 млн * 1 байт + 11.04 млн * 20 байт + 4.56 млн * 12 байт)  -->
